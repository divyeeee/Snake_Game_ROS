...
def game_over_condition():
    global Turtle_List
    #checks the distance of the first turtle in the list with all other turtles and if the distance is less than the minimum distance then we can say that turtle has crashed or snake has eaten himself
    for i in range(len(Turtle_List)):
        #for j in range(len(Turtle_List)):
            if i != 0 : 
                dist = math.sqrt((Turtle_List[i].turtle_pose.x - Turtle_List[0].turtle_pose.x)**2 + (Turtle_List[i].turtle_pose.y - Turtle_List[0].turtle_pose.y)**2)
                if dist < Min_Dis:  
                    return True  
    return False  
